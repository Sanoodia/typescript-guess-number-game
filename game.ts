...
const promptGuess = async (attempts: number) => {
  const prompt: {
    guess: string;
...
const startGame = () => {
  ...
  function continueGame() {
    ...
    if (lives) {
      ...
      console.log(
        chalk.gray.bold(`Actuall Number is  ${actualNumber}`)
      );
    }
  }
  return continueGame;
